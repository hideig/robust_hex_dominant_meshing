...
#include "Combine/exact_cell_evaluation.h"
#include "Combine/hxt_api.h"
#include "Combine/hxt_mesh.h"
#include "hierarchy.h"
/**
...
namespace HXTCombine {
  ...
  */
  ...
  void myReadFileMESH(const MatrixXf &V_, const MatrixXu &F_, const MatrixXu &T, MeshStore& mesh, bool read_triangles = true);
  void myreadFileAbaqusMESH(const std::string& filename, MeshStore& mesh, bool read_triangles = false);
  /**
  *  Common accessor to mesh storage.
  *
  *  The class neither own or can modify the arrays it points to.
  */
  ...
  public:
    ...
    {
      return orient3dd(point(v0), point(v1), point(v2), point(v3)) > 0;
    }
    ...
    trindex tetFacet(TetIndex t, TetFacetIndex lf) const {
      VertexIndex v0 = vertex(t, Teta::facetVertex[lf][0]);
      VertexIndex v1 = vertex(t, Teta::facetVertex[lf][1]);
      VertexIndex v2 = vertex(t, Teta::facetVertex[lf][2]);
      return trindex(v0, v1, v2);
    }
    bindex tetEdge(TetIndex t, TetEdgeIndex le) const {
      VertexIndex v0 = vertex(t, Teta::edgeVertex[le][0]);
      VertexIndex v1 = vertex(t, Teta::edgeVertex[le][1]);
      return bindex(v0, v1);
    }
  ...
  public:
    ...
    */
    ...
    inline vector<VertexIndex> verticesAroundVertices(VertexIndex v0, VertexIndex v1) const
    {
      ...
      std::set_intersection(first.begin(), first.end(), second.begin(), second.end(), std::back_inserter(result), std::greater<VertexIndex>());
      return result;
    }
    ...
    double hexCornerQuality(VertexIndex v0, VertexIndex v1, VertexIndex v2, VertexIndex v3) const {
      return cellCornerQuality<Hexa>(point(v0), point(v1), point(v2), point(v3));
    }
