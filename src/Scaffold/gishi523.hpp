...
public:
  ...
  KDTree()
    : root_(nullptr){};
  KDTree(const std::vector<PointT>& points)
    : root_(nullptr)
  {
    build(points);
  }
  ...
  PointT point(int id) const { return points_[id]; }
...
protected:
  ...
  public:
    ...
    {
      auto it =
        std::find_if(std::begin(elements_),
                     std::end(elements_),
   ...
   */
  void nnSearchRecursive(const PointT& query,
                         const Node* node,
                         int* guess,
   ...
   */
  void knnSearchRecursive(const PointT& query,
                          const Node* node,
                          KnnQueue& queue,
                          int k) const
  {
  ...
  /** @brief Searches neighbors within radius.
   */
  void radiusSearchRecursive(const PointT& query,
                             const Node* node,
                             std::vector<int>& indices,
                             double radius) const
  {
